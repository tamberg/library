...
- Search: to find items by group or item name (search directory and file names)
- Request: to borrow this item for n weeks (issue with item name, user, timestamp)
- Availability check: list requests for this item (issue list filtered by item name)
- User: borrows items (GitHub user with right to search and read files, open an issue)
- Admin: lends items (GitHub user with right to edit and add files to repository, tag an issue)
